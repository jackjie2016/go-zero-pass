...
```cassandraql
cat >> /etc/hosts <<EOF
192.168.119.139 ceph01 
192.168.119.140 ceph02 
192.168.119.141 ceph03 
EOF
```
...
```cassandraql
cephadm bootstrap --mon-ip 192.168.119.139 
    -- 172.31.96.70
```
此命令将会进行以下操作：
...
安装日志如下为成功
```cassandraql
[root@ceph01 ~]# cephadm bootstrap --mon-ip 192.168.119.136
...
添加ceph 02 节点
```cassandraql
ceph orch host add ceph02 192.168.119.140
ceph orch host add ceph03 192.168.119.141
...
ceph orch apply prometheus --placement="ceph01 ceph02 ceph03"    
```
查看Service状态。
```cassandraql
[root@ceph01 ~]# ceph orch ls
...
spec:
  pool: iscsi_pool
  trusted_ip_list: "192.168.119.139,192.168.119.140,192.168.119.141"
  api_user: admin
  api_password: admin
  api_secure: false
