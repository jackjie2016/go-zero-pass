...
import (
	...
	"go-zero-pass/app/service/k8s/svc/model/repository"
	"go-zero-pass/app/service/k8s/svc/rpc/svc"
...
func (u *SvcDataService) CreateSvcToK8s(svcInfo *svc.SvcInfo) (err error) {
	...
	if _, err = u.K8sClientSet.CoreV1().Services(svcInfo.SvcNamespace).Get(context.TODO(), svcInfo.SvcName, v12.GetOptions{}); err != nil {
		...
		logx.Error("Service " + svcInfo.SvcName + "已经存在")
		return errors.New("Service " + svcInfo.SvcName + "已经存在")
	}

}
func (u *SvcDataService) InitListFromK8s(Namespace string) (err error) {
	ServiceList := &v1.ServiceList{}

	if ServiceList, err = u.K8sClientSet.CoreV1().Services(Namespace).List(context.TODO(), v12.ListOptions{}); err != nil {
		logx.Error(err)
		return err
	} else {
		logx.Info("实现k8s 同步到db中")
		logx.Info(ServiceList)
...
func (u *SvcDataService) setService(svcInfo *svc.SvcInfo) *v1.Service {
	...
	service.ObjectMeta = v12.ObjectMeta{
		...
		ResourceVersion: "10000001",
		Labels: map[string]string{
			"app-name": svcInfo.SvcPodName,
			"author":   "JackJie",
		},
		Annotations: map[string]string{
			"k8s/generated-by-jack":  "由JackJie创建",       //这个自定义字段
			"k8s/generated-by-email": "610841147@qq.com", //这个自定义字段
		},
	}
...
func (u *SvcDataService) UpdateSvcToK8s(svcInfo *svc.SvcInfo) (err error) {
	...
	if _, err = u.K8sClientSet.CoreV1().Services(svcInfo.SvcNamespace).Get(context.TODO(), svcInfo.SvcName, v12.GetOptions{}); err != nil {
		...
		logx.Info("Service " + svcInfo.SvcName + "更新成功")
		return nil
	}
}
...
func (u *SvcDataService) DeleteFromK8s(svc *model.Svc) (err error) {
	if err = u.K8sClientSet.CoreV1().Services(svc.SvcNamespace).Delete(context.TODO(), svc.SvcName, v12.DeleteOptions{}); err != nil {
		...
		logx.Info("删除Service ID：" + strconv.FormatInt(svc.ID, 10) + "成功！")
	}
	return
}
