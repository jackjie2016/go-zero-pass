...
type IVolumeDataService interface {
	...
	UpdateVolumeToK8s(*volume.VolumeInfo) error
...
func (u *VolumeDataService) DeleteVolumeFromK8s(volume *model.Volume) (err error) {
	...
	if err = u.K8sClientSet.CoreV1().PersistentVolumeClaims(volume.VolumeNamespace).Delete(context.TODO(), volume.VolumeName, v13.DeleteOptions{}); err != nil {
		...
		logx.Info("删除存储ID" + strconv.FormatInt(volume.ID, 10) + " 成功！")
	}
	return
}
...
func (u *VolumeDataService) CreateVolumeToK8s(info *volume.VolumeInfo) (err error) {
	...
	if _, err = u.K8sClientSet.CoreV1().PersistentVolumeClaims(info.VolumeNamespace).Get(context.TODO(), info.VolumeName, v13.GetOptions{}); err != nil {
		...
		logx.Error("存储空间" + info.VolumeName + " 已经存在")
		return errors.New("存储空间" + info.VolumeName + " 已经存在")
	}
}

// 更新存储到k8s中
func (u *VolumeDataService) UpdateVolumeToK8s(info *volume.VolumeInfo) (err error) {
	volume := u.setVolume(info)
	if k8svolume, err := u.K8sClientSet.CoreV1().PersistentVolumeClaims(info.VolumeNamespace).Get(context.TODO(), info.VolumeName, v13.GetOptions{}); err == nil {
		//如果存储存在
		logx.Info(k8svolume)
		if _, err = u.K8sClientSet.CoreV1().PersistentVolumeClaims(info.VolumeNamespace).Update(context.TODO(), volume, v13.UpdateOptions{}); err != nil {
			logx.Error(err)
			return err
		}
		logx.Info("存储更新成功")
		...
		logx.Info(k8svolume)
		logx.Error("存储空间" + info.VolumeName + " 不存在")
		return errors.New("存储空间" + info.VolumeName + " 不存在")
	}
}
